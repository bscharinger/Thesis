...
double eig3volume( int nlhs, double *plhs[], int nrhs, const int *prhs[] ) {
	...
	Deiga = (double*)&(plhs[0]); Deigb = (double*)&(plhs[1]); Deigc = (double*)&(plhs[2]);
	if(nlhs==6) {
		/* Main direction (larged eigenvector) */
		Dvecx = (double*)&(plhs[3]); Dvecy = (double*)&(plhs[4]); Dvecz = (double*)&(plhs[5]);
	}
